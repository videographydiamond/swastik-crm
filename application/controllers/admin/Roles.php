...
class Roles extends BaseController
{
    ...
    public function __construct()
    {
       ...
       $this->load->model('admin/module_role_model');

         $this->global['module_id']      = get_module_byurl('admin/roles');
    ...
    public function index()
    {
        ...
        $this->loadViews("admin/roles/list", $this->global, $data , NULL);
    ...
    public function addnew()
    {
        ...
        $this->loadViews("admin/roles/addnew", $this->global, $data , NULL);
    ...
    public function insertnow()
    {
        ...
        else
        {
            ...
            $returnData     = $this->role_model->findDynamic($where);
            if(!empty($returnData)){
               $this->session->set_flashdata('error', $form_data['name'].' already Exist.');
    			...
    			$result = $this->role_model->save($insertData);
                if($result > 0)
                {
                ...
                { 
                    $this->session->set_flashdata('error', 'Role Addition failed');
                     redirect(base_url().'admin/roles/addnew');
                }
            }   
            redirect('admin/roles/addnew');
          }  
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $currentObj) {
            ...
            $row[] = $dateAt;
            $row[] = $btn;
            $row[] = '<a class="btn btn-sm btn-info" href="'.base_url().'admin/roles/edit/'.$currentObj->id.'" title="Edit" ><i class="fa fa-pen"></i></a>&nbsp;<a class="btn btn-sm btn-danger deletebtn" href="#" data-userid="'.$currentObj->id.'"><i class="fa fa-trash"></i></a>';
            $data[] = $row;
        }
    ...
    public function edit($id = NULL)
    {
        ...
        $data['edit_data'] = $this->role_model->find($id);
        $this->global['pageTitle'] = 'Role Edit ';
        $this->loadViews("admin/roles/edit", $this->global, $data , NULL);
      ...
      function delete()
    {
        ...
        $result = $this->role_model->delete($delId); 
            ...
            if(!empty($returnData)){
               $this->session->set_flashdata('error', $form_data['name'].' already Exist.');
                ...
                { 
                    ...
                    redirect(base_url().'admin/roles/edit/'.$form_data['id']);
                }
            }
    ...
    public function update_role_module()
    {
        ...
        $company_id = $this->session->userdata('company_id');
            ...
            $where['field']         = 'id';
            $update_status             = $this->module_role_model->findDynamic($where);
            ...
            if(!empty($update_status))
            {
                foreach ($update_status as $key => $value)
                {
                    ...
                    $this->module_role_model->save($insertData);
                }
            }
         ...
         foreach ($module_checked  as $key => $value) {
                ...
                $where['role_id']       = $role_id ;
                $returnData             = $this->module_role_model->findDynamic($where);
                ...
                $result = $this->module_role_model->save($insertData);
         }
             $this->session->set_flashdata('success', 'Role Module successfully Updated');
            redirect(base_url().'admin/roles/edit/'.$role_id);
