...
class Root_dtl extends BaseController
{
    ...
    public function index()
    {
        ...
        $this->loadViews("admin/root_dtl/list", $this->global, $data , NULL);
    ...
    public function addnew()
    {
        ...
        $this->loadViews("admin/root_dtladdnew", $this->global, $data , NULL);
    ...
    public function insertnow()
    {
        ...
        else
        {
            ...
            $returnData     = $this->root_dtl_model->findDynamic($where);
            if(!empty($returnData)){
               $this->session->set_flashdata('error', $form_data['name'].' already Exist.');
    			...
    			$result = $this->root_dtl_model->save($insertData);
                if($result > 0)
                {
                ...
                { 
                    $this->session->set_flashdata('error', 'Role Addition failed');
                     redirect(base_url().'admin/root_dtladdnew');
                }
            }   
            redirect('admin/root_dtladdnew');
          }  
    ...
    {
		$list = $this->root_dtl_model->get_datatables();
   
		$data = array();
        $no =(isset($_POST['start']))?$_POST['start']:'';
        foreach ($list as $currentObj) {

             $temp_date = $currentObj->create_at;
            ...
            $row[] = ucwords($currentObj->pin);
            $row[] = ucwords($currentObj->orders);
            $row[] = $dateAt;
            $row[] = $btn;
            $row[] = '<a class="btn btn-sm btn-info" href="'.base_url().'admin/root_dtledit/'.$currentObj->id.'" title="Edit" ><i class="fa fa-pen"></i></a>&nbsp;<a class="btn btn-sm btn-danger deletebtn" href="#" data-userid="'.$currentObj->id.'"><i class="fa fa-trash"></i></a>';
            $data[] = $row;
        }
 
        $output = array(
                        "draw" => (isset($_POST['draw']))?$_POST['draw']:'',
                        "recordsTotal" => $this->root_dtl_model->count_all(),
                        "recordsFiltered" => $this->root_dtl_model->count_filtered(),
                        "data" => $data,
                );
    ...
    public function edit($id = NULL)
    {
        ...
        $data['edit_data'] = $this->root_dtl_model->find($id);
        $this->global['pageTitle'] = 'Role Edit ';
        $this->loadViews("admin/root_dtledit", $this->global, $data , NULL);
      ...
      function delete()
    {
        ...
        $result = $this->root_dtl_model->delete($delId); 
            ...
            if(!empty($returnData)){
               $this->session->set_flashdata('error', $form_data['name'].' already Exist.');
                ...
                { 
                    ...
                    redirect(base_url().'admin/root_dtledit/'.$form_data['id']);
                }
            }
    ...
    public function update_role_module()
    {
        ...
        $company_id = $this->session->userdata('company_id');
            ...
            $where['field']         = 'id';
            $update_status             = $this->module_root_dtl_model->findDynamic($where);
            ...
            if(!empty($update_status))
            {
                foreach ($update_status as $key => $value)
                {
                    ...
                    $this->module_root_dtl_model->save($insertData);
                }
            }
         ...
         foreach ($module_checked  as $key => $value) {
                ...
                $where['role_id']       = $role_id ;
                $returnData             = $this->module_root_dtl_model->findDynamic($where);
                ...
                $result = $this->module_root_dtl_model->save($insertData);
         }
             $this->session->set_flashdata('success', 'Role Module successfully Updated');
            redirect(base_url().'admin/root_dtledit/'.$role_id);
